...
export const entityIcon = (stateObj: HomeAssistantEntity, config: RoomCardEntity | RoomCardConfig, hass: HomeAssistant) => {
    ...
    if((config.icon as RoomCardIcon).state_on) return renderCustomStateIcon(stateObj, config.icon as RoomCardIcon);
    if((config.icon as RoomCardIcon).conditions) return renderConditionIcons(stateObj, config, hass);
    if((config.icon as RoomCardIcon).template?.icon) return evalTemplate(hass, stateObj, (config.icon as RoomCardIcon).template!.icon);
}
...
export const renderIcon = (stateObj: HomeAssistantEntity, config: RoomCardEntity | RoomCardConfig, hass: HomeAssistant, classes? : string) : HTMLTemplateResult => {
    ...
    return html`<state-badge
        ...
        .stateColor="${(config as any).state_color}"
        style="${customStyling ?? entityStyles(isObject(customIcon) ? (customIcon as EntityCondition).styles : null, hass.states[(config as any).entity], hass)}"
    ></state-badge>`;
}
...
/**
 * Haupt-Entity im Header (Icon oder State)
 */
export const renderMainEntity = (entity: RoomCardEntity | undefined, config: RoomCardConfig, hass: HomeAssistant) : HTMLTemplateResult => {
    ...
    return html`<div
        ...
        ${config.entities?.length === 0 || (config as any).icon
...
export const renderTitle = (config: RoomCardConfig, hass: HomeAssistant, element: LitElement, entity?: RoomCardEntity) : HTMLTemplateResult => {
    ...
    return html`<div class="title${(hasConfigAction ? ' clickable' : '')}" @action=${_handleAction}
...
/**
 * INFO ENTITIES → nur ICON anzeigen (kein Name, kein State)
 */
export const renderInfoEntity = (entity: RoomCardEntity, hass: HomeAssistant, element: LitElement) : HTMLTemplateResult => {
    ...
    // Nur Icon – State/Name werden hier nicht gerendert
    return html`<div class="state entity icon-entity"
...
export const renderEntity = (entity: RoomCardEntity, hass: HomeAssistant, element: LitElement) : HTMLTemplateResult => {    
    ...
    // NAME über dem Icon, STATE darunter (falls show_state true)
